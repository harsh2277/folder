...
const STORAGE_KEY = 'lightmap_designer_notification_state';
    ...
    async function fetchRealEvents() {
      try {
        ...
        const { data: myProjects } = await supabase
          .from('projects')
          .select('id, project_name, project_id_serial, client_name, status, created_at, project_notes')
        ...
        (myProjects || []).forEach((p: any) => {
          items.push({
            id: `project_${p.id}`,
            title: `Assigned project: ${p.project_name}`,
        ...
        (revisions || []).forEach((rev: any) => {
          ...
          items.push({
            ...
            href: proj ? `/designer/projects/${proj.id}` : undefined,
          });
        });
