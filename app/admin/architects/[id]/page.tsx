  ...
  if (!architect) {
    return (
      <div className="text-center py-12">
        <h2 className="text-xl font-medium text-neutral-900">Architect profile not found.</h2>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center space-x-3.5">
          ...
          <div>
            <h2 className="text-xl font-medium text-neutral-900 font-sans">{architect.name}</h2>
        ...
        <div className="flex items-center space-x-2">
          <a
            ...
            className="px-4 py-2 border border-neutral-200 hover:bg-neutral-50 bg-white font-medium text-sm rounded-md transition-colors flex items-center space-x-1.5 text-neutral-700"
          >
      ...
      <div className="bg-white border border-neutral-200 rounded-md p-5">
        <h3 className="text-sm font-medium text-neutral-900 mb-4 font-sans">Contact & Account Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
          <div>
            <span className="text-neutral-400 font-medium block mb-0.5">Email Address</span>
            <span className="text-neutral-800 font-medium">{architect.email}</span>
          </div>
          <div>
            <span className="text-neutral-400 font-medium block mb-0.5">Phone Number</span>
            <span className="text-neutral-800 font-medium font-sans">{architect.mobile_number || 'Not Provided'}</span>
          </div>
          <div>
            <span className="text-neutral-400 font-medium block mb-0.5">Partner Since</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-neutral-200 rounded-md p-5 flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-sm font-medium text-neutral-400 block">Completed</span>
      ...
      <div className="space-y-4">
        <div>
          <h3 className="text-xl font-medium text-neutral-900 font-sans">Portfolio Projects</h3>
                ...
                {projects.map((proj) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4 first:pl-5 last:pr-5">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-sm font-medium border ${proj.payment_status === 'paid' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : proj.payment_status === 'failed' ? 'bg-rose-50 border-rose-100 text-rose-700' : 'bg-amber-50 border-amber-100 text-amber-700' }`}>
                        {proj.payment_status}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 first:pl-5 last:pr-5">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-sm font-medium border w-fit whitespace-nowrap ${proj.status === 'Approved' || proj.status === 'Closed' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : proj.status === 'In Design' ? 'bg-indigo-50 border-indigo-100 text-indigo-700' : proj.status === 'Under Review' ? 'bg-blue-50 border-blue-100 text-blue-700' : 'bg-neutral-50 border-neutral-200 text-neutral-600' }`}>
                        {proj.status}
                      </span>
                    </td>
