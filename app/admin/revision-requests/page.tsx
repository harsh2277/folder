  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto font-sans">
      <div className="border-b border-neutral-100 pb-6">
        <h1 className="text-xl font-medium text-neutral-900 tracking-tight">Revision Requests Review</h1>
        <p className="text-sm text-neutral-500 mt-1 font-medium">Review and manage design modifications requested by architects</p>
      </div>

      {actionMessage && (
        <div className={`p-4 rounded text-sm font-medium ${actionMessage.includes('Successfully') ? 'bg-emerald-50 text-emerald-800 border border-emerald-100' : 'bg-rose-50 text-rose-800 border border-rose-100'}`}>
          {actionMessage}
        </div>
      )}
                ...
                {requests.map((req) => {
                  ...
                  return (
                    <tr key={req.id} className="hover:bg-neutral-50/50 transition-colors">
                      <td className="py-4 px-4 font-medium text-neutral-400">{new Date(req.created_at).toLocaleDateString()}</td>
                      <td className="py-4 px-4">
                        <p className="font-medium text-neutral-900">{req.projects?.project_name}</p>
                        <p className="text-sm text-neutral-450 mt-0.5">{req.projects?.project_id_serial || 'N/A'}</p>
                      </td>
                      <td className="py-4 px-4 text-neutral-600">{req.architect?.name || 'Assigned Architect'}</td>
                      <td className="py-4 px-4 text-neutral-750 font-medium leading-relaxed max-w-xs truncate" title={architectRequest}>
                        {architectRequest}
                      </td>
                      <td className="py-4 px-4 text-center">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-sm font-medium border ${ req.status === 'approved' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : req.status === 'completed' ? 'bg-blue-50 border-blue-100 text-blue-700' : req.status === 'declined' ? 'bg-rose-50 border-rose-100 text-rose-700' : 'bg-amber-50 border-amber-100 text-amber-700' }`}>
                          {req.status === 'completed' ? '✓ Resolved' : req.status}
                        </span>
                      </td>
                        ...
                        {req.status === 'pending' ? (
                          <>
                            <button
                              ...
                              className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-750 text-white font-medium text-sm rounded transition-colors cursor-pointer"
                            >
                              Approve
                            </button>
                            <button
                              onClick={() => handleUpdateStatus(req.id, req.project_id, 'declined')}
                              className="px-2.5 py-1.5 bg-rose-600 hover:bg-rose-750 text-white font-medium text-sm rounded transition-colors cursor-pointer"
                            >
                              Decline
                            </button>
                          </>
                        ) : (
                          <span className="text-sm text-neutral-450 font-medium">Reviewed</span>
                        )}
                      </td>
                    </tr>
                  );
