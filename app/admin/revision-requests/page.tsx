  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto font-sans">
      <div className="border-b border-neutral-100 pb-6">
        <h1 className="text-2xl font-black text-neutral-900 tracking-tight">Revision Requests Review</h1>
        <p className="text-xs text-neutral-500 mt-1 font-semibold uppercase tracking-wider">Review and manage design modifications requested by architects</p>
      </div>

      {actionMessage && (
        <div className={`p-4 rounded text-xs font-bold ${actionMessage.includes('Successfully') ? 'bg-emerald-50 text-emerald-800 border border-emerald-100' : 'bg-rose-50 text-rose-800 border border-rose-100'}`}>
          {actionMessage}
        </div>
      )}
              ...
              <tbody className="divide-y divide-neutral-100 text-neutral-700 font-semibold">
                {requests.map((req) => (
                  <tr key={req.id} className="hover:bg-neutral-50/50 transition-colors">
                    <td className="py-4 px-4 font-mono font-bold text-neutral-400">{new Date(req.created_at).toLocaleDateString()}</td>
                    <td className="py-4 px-4">
                      <p className="font-bold text-neutral-900">{req.projects?.project_name}</p>
                      <p className="text-[10px] text-neutral-400 font-mono mt-0.5">{req.projects?.project_id_serial || 'N/A'}</p>
                    </td>
                      ...
                      {req.status === 'pending' ? (
                        <>
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-750 text-white font-bold text-[10px] rounded transition-colors uppercase cursor-pointer"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() => handleUpdateStatus(req.id, req.project_id, 'declined')}
                            className="px-2.5 py-1.5 bg-rose-600 hover:bg-rose-750 text-white font-bold text-[10px] rounded transition-colors uppercase cursor-pointer"
                          >
                            Decline
                          </button>
                        </>
                      ) : (
                        <span className="text-[10px] text-neutral-400 font-bold uppercase">Reviewed</span>
                      )}
                    </td>
                  </tr>
                ))}
